...
def extract_epoch_number(filename):
    """Extract epoch number from filename like 'pytorch_samples_epoch_5.png'"""
...
def create_loss_plot(csv_path, current_epoch, max_epoch):
    ...
    train_recon = [data['Train_Recon'][i] for i in indices]
    val_recon = [data['Val_Recon'][i] for i in indices]
    ...
    axes[1].plot(epochs, train_recon, 'b-', label='Train', linewidth=2)
    axes[1].plot(epochs, val_recon, 'r-', label='Val', linewidth=2)
    axes[1].set_ylabel('Reconstruction', fontsize=10)
    axes[1].legend(loc='upper right', fontsize=9)
    axes[1].grid(True, alpha=0.3)
    axes[1].set_xlim(0, max_epoch)

    # Plot 3: KL Divergence
    axes[2].plot(epochs, train_kl, 'b-', label='Train', linewidth=2)
    axes[2].plot(epochs, val_kl, 'r-', label='Val', linewidth=2)
    axes[2].set_xlabel('Epoch', fontsize=10)
    axes[2].set_ylabel('KL Divergence', fontsize=10)
...
def create_training_video(
    ...
    pattern = os.path.join(images_dir, "pytorch_samples_epoch_*.png")
    ...
    for img_file, epoch in zip(image_files, epochs):
        ...
        if csv_path:
            ...
            # Create blank plot placeholder
            plot_img = Image.new('RGB', (600, 800), color='white')
